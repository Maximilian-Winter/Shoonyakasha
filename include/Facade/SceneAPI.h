...
public:
    ...
    /// Draw order among sprites and labels. Lower values are drawn first, so a
    /// label drawn over a panel needs a higher key than the panel. Labels
    /// default to 0.
    void setTextSortKey(EntityHandle entity, uint32_t sortKey);

    /// Show or hide a label. Hiding is the supported way to remove text: the
    /// glyph entities a label generates are not linked to it by hierarchy, so
    /// destroying the label entity leaves them on screen.
