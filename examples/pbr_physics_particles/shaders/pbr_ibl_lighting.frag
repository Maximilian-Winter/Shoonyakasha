...
void main() {
    ...
    // ══════════════════════════════════════════════════════════════════
    // IBL DEBUG: Set to 1 to output ONLY the raw irradiance sample
    //            Set to 2 to output irradiance * albedo (diffuse IBL)
    //            Set to 3 to output prefiltered specular
    ...
    #define IBL_DEBUG 0
        ...
        return;
    #elif IBL_DEBUG == 2
        outColor = vec4(irradiance * albedo, 1.0);
        return;
    #elif IBL_DEBUG == 3
        const float MAX_REFLECTION_LOD_DBG = 4.0;
        vec3 R_dbg = reflect(-V, N);
        vec3 pf = textureLod(prefilterMap, R_dbg, roughness * MAX_REFLECTION_LOD_DBG).rgb;
        outColor = vec4(pf, 1.0);
        return;
    #endif
